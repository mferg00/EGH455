# modified from https://github.com/cyberj0g/logo-detector
import traceback
import cv2
import numpy as np
...
class Symbol:
    ...
    def __init__(self, path='dell.png'):
        self.train_features = self.get_features(cv2.imread(path))
    ...
    def get_features(self, frame):
        ...
        return kps, descs, frame.shape[:2][::-1]

    def find(self, frame):
        train_kps, train_descs, shape = self.train_features
        # get features from input image
        kps, descs, _ = self.get_features(frame)
