...
function randomMarker(id) {

	var el = document.getElementById('marker');
	el.innerHTML += '<div class="labels">' + 
		(new ArucoMarker(id).toSVG('150px')) + '<h3>ID: ' + String(id) + '</h3></div>';
}
	...
	$.ajax({
		...
		success: function(res) {
			...
			for(var i = 0; i < sensors.length; i++){
				createChart(sensors[i],res,i+1,labels[i]);
			}
...
function ajax_labels(){
	$.ajax({
		url: "http://localhost:5000/get_labels",
		type: 'GET',
		success: function(res){
			if (res.length > 0){
				if (parseInt(res[0]) == 1){
					document.getElementById("dangerous").innerHTML = "Detected";
				}
				else{
					document.getElementById("dangerous").innerHTML = "N/A";
				}
				if (parseInt(res[1]) == 1){
					document.getElementById("corrosive").innerHTML = "Detected";
				}
				else{
					document.getElementById("corrosive").innerHTML = "N/A";
				}
				document.getElementById('marker').innerHTML = '';
				for(var i = 0; i < res[2].length; i++){
					randomMarker(res[2][i]);
				}
			}
		}
	});
