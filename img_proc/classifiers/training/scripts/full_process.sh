...
{   
    ...
    mkdir -p "$1"/stages
    rm -rf "$1"/stages/*
    opencv_traincascade -data "$1"/stages -vec "$1"/positives.vec -bg bg.txt -numPos 200 -numNeg 100 -numStages ${stages:-3} -maxFalseAlarmRate 0.1 -w 20 -h 20
    cp "$1"/stages/cascade.xml ../"$1".xml
}
...
# 3 stages took a few seconds, but 15 stages took about 10-20 minutes for me 
...
dotraining corrosive 15
dotraining dangerous 15
